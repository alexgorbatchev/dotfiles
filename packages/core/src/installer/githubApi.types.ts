 ...
 *
 * Implementations of this interface are responsible for fetching release
 * information, handling authentication, and managing API rate limits.
 *
 * @public
 */
export interface IGitHubApiClient {
   ...
   * @returns A promise that resolves with the {@link GitHubRelease} object for
   *          the latest release. Throws an error if no releases are found or
   ...
   * @returns A promise that resolves with the {@link GitHubRelease} object for
   *          the specified tag. Throws an error if the release is not found or
   *          if the API request fails.
   */
  getReleaseByTag(owner: string, repo: string, tag: string): Promise<GitHubRelease>;

  /**
   * Fetches all releases for a given repository, with optional pagination and
   * filtering.
   *
   * @param owner - The owner of the repository.
   * @param repo - The name of the repository.
   ...
   * @param options.includePrerelease - If `true`, pre-releases will be included.
   *                                    Defaults to `false`.
  ...
  /**
   * Fetches a release that satisfies a given version constraint (e.g., a SemVer range).
   *
   * This method typically involves fetching multiple releases and finding the
   * best match for the specified constraint.
   *
   * @param owner - The owner of the repository.
   * @param repo - The name of the repository.
   * @param constraint - A version constraint string (e.g., `^1.2.3`, `~2.0.x`).
   * @returns A promise that resolves with the {@link GitHubRelease} that best
   *          matches the constraint, or `null` if no matching release is found.
   */
