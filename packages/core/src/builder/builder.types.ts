...
/**
 * Context available to shell configurator callbacks.
 * Provides version information that is only available after installation.
 * For other context properties (toolDir, currentDir, projectConfig, etc.),
 * use the outer ctx from defineTool.
 */
export interface IShellConfiguratorContext {
  /**
   * The installed version of the tool.
   * Available after successful installation; may be undefined for externally managed tools
   * or when version resolution fails.
   *
   * @example 'v0.26.1'
   * @example '1.2.3'
   */
...
export type ShellConfiguratorCallback = (
  shell: IShellConfigurator,
  ctx: IShellConfiguratorContext
) => ShellConfiguratorSyncResult;
export type ShellConfiguratorAsyncCallback = (
  shell: IShellConfigurator,
  ctx: IShellConfiguratorContext
) => ShellConfiguratorAsyncResult;
