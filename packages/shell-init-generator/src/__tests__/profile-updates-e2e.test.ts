...
describe("Profile Updates E2E Tests", () => {
  ...
  describe("Complete Profile Update Workflow", () => {
    it("should generate shell scripts and update all existing profile files", async () => {
      ...
      const expectedContentByShell = {
        bash: [
          `export PATH="${testDirs.paths.targetDir}:$PATH"`,
          'alias g="lazygit"',
          "export LAZYGIT_CONFIG_FILE",
          "export FZF_DEFAULT_OPTS",
        ],
        powershell: [
          `$env:PATH = "${testDirs.paths.targetDir};$env:PATH"`,
          "Set-Alias g lazygit",
          "$env:LAZYGIT_CONFIG_FILE",
        ],
        zsh: [
          `export PATH="${testDirs.paths.targetDir}:$PATH"`,
          'alias g="lazygit"',
      ...
      for (const { shell, path: scriptPath } of generatedFiles) {
        ...
        const expectedSnippets = expectedContentByShell[shell as keyof typeof expectedContentByShell];
        for (const snippet of expectedSnippets) {
          expect(scriptContent).toContain(snippet);
        }
      }
      ...
      for (const profileUpdate of result!.profileUpdates!) {
        ...
        const expectedSourceLineByShell = {
          bash: `source "${expectedScriptPath}"`,
          powershell: `. "${expectedScriptPath}"`,
          zsh: `source "${expectedScriptPath}"`,
        } as const;

        expect(profileContent).toContain(
          expectedSourceLineByShell[profileUpdate.shellType as keyof typeof expectedSourceLineByShell],
        );
