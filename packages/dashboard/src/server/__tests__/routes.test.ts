...
describe('Dashboard API Routes', () => {
  ...
  describe('getRecentTools', () => {
    test('returns empty array when no tool config files exist', async () => {
      ...
      await fs.mkdir('/home/user/.dotfiles/tools', { recursive: true });

      const result = await api.getRecentTools();
    ...
    test('returns recently added tool config files', async () => {
      ...
      await fs.mkdir('/home/user/.dotfiles/tools', { recursive: true });
      await fs.writeFile('/home/user/.dotfiles/tools/fzf.tool.ts', 'export default {}');
      await fs.writeFile('/home/user/.dotfiles/tools/bat.tool.ts', 'export default {}');
      ...
      const names = result.data?.tools.map((t) => t.name).toSorted();
    ...
    test('returns tools sorted by creation time descending', async () => {
      ...
      // Create first file
      await fs.writeFile('/home/user/.dotfiles/tools/first.tool.ts', 'export default {}');

      // Small delay to ensure different timestamps
      await new Promise((resolve) => setTimeout(resolve, 10));

      // Create second file
      await fs.writeFile('/home/user/.dotfiles/tools/second.tool.ts', 'export default {}');
    ...
    test('limits results to specified count', async () => {
      ...
      // Create 5 tool files
      for (let i = 0; i < 5; i++) {
        await fs.writeFile(`/home/user/.dotfiles/tools/tool-${i}.tool.ts`, 'export default {}');
        await new Promise((resolve) => setTimeout(resolve, 5));
      }

      const result = await api.getRecentTools(3);
    ...
    test('includes tool files from subdirectories', async () => {
      await fs.mkdir('/home/user/.dotfiles/tools/cli', { recursive: true });
      await fs.mkdir('/home/user/.dotfiles/tools/dev', { recursive: true });

      await fs.writeFile('/home/user/.dotfiles/tools/fzf.tool.ts', 'export default {}');
      await fs.writeFile('/home/user/.dotfiles/tools/cli/ripgrep.tool.ts', 'export default {}');
      await fs.writeFile('/home/user/.dotfiles/tools/dev/node.tool.ts', 'export default {}');
