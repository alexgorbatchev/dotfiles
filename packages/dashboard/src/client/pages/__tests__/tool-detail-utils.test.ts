...
function createMockToolDetail(
  overrides: Partial<ISerializableToolConfig> & { name: string; },
): IToolDetail {
  return {
    config: {
      name: overrides.name,
...
describe('buildBinaryToToolMap', () => {
  ...
  test('handles tools without binaries', () => {
    const tools = [
      createMockToolDetail({ name: 'tool-d', binaries: undefined }),
    ];
...
describe('findDependentTools', () => {
  ...
  test('handles tools without dependencies', () => {
    const tools = [
      createMockToolDetail({ name: 'tool-a', binaries: ['bin-a'], dependencies: undefined }),
    ];
