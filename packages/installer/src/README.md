...
├── types.ts                     # Installer interface and types
├── installers/                  # Method-based installers (installFrom*.ts)
├── steps/                       # Installation step implementations
└── utils/                       # Shared utilities (Pipeline, Hooks, etc.)
```
...
```typescript
import { InstallationPipeline } from '@dotfiles/installer';
import { createArchivePipeline } from '@dotfiles/installer/utils';
...
```typescript
import { executeAfterDownloadHook } from '@dotfiles/installer/utils';
