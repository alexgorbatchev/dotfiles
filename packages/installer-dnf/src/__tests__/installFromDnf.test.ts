...
import type { DnfToolConfig } from "../schemas";
...
function createMockContext(toolConfig: DnfToolConfig, mockShell: IShell): IInstallContext {
  return {
    projectConfig: {
      paths: {
        binariesDir: "/bin",
...
describe("installFromDnf", () => {
  ...
  it("installs latest package without refreshing metadata by default", async () => {
    const commands: string[] = [];
    const results: MockShellResult[] = [
      createShellResult(""),
      createShellResult("/usr/bin/rg\n"),
      createShellResult("13.0.0-1.fc40"),
    ];
    ...
    const result = await installFromDnf("ripgrep", toolConfig, context, undefined, logger, shell, shell);

    assert(result.success);
    expect(result.version).toBe("13.0.0-1.fc40");
  ...
  it("installs an exact package version and refreshes metadata when requested", async () => {
    const commands: string[] = [];
  ...
  it("runs makecache and install through sudo when configured", async () => {
    ...
    expect(spawnMock.mock.calls).toEqual([
      [
        {
          cmd: ["sudo", "-p", "Please enter your password to continue:", "--", "dnf", "makecache"],
          cwd: "/staging/dir",
          env: process.env,
