...
import { type IMockSetup, setupMockGiteaApiClient } from "./helpers/sharedGiteaApiClientTestSetup";
...
describe("GiteaApiClient", () => {
  ...
  describe("getLatestRelease", () => {
    it("should fetch and return the latest release", async () => {
      mocks.mockDownloader.download.mockResolvedValue(Buffer.from(JSON.stringify(latestReleaseFixture)));
    ...
    it("should return null if the release is not found (404)", async () => {
      ...
      mocks.mockDownloader.download.mockRejectedValue(new NotFoundError(mocks.logger, url, new Error("404")));
  ...
  describe("getAllReleases", () => {
    it("should fetch and map all releases", async () => {
      ...
      mocks.mockDownloader.download.mockResolvedValueOnce(Buffer.from(JSON.stringify(allReleasesFixture.default)));
  ...
  describe("getLatestReleaseTags", () => {
    it("should return tag names from releases", async () => {
      const allReleasesFixture = await import("./fixtures/allReleasesPage1.json");
      mocks.mockDownloader.download.mockResolvedValue(Buffer.from(JSON.stringify(allReleasesFixture.default)));
  ...
  describe("caching", () => {
    ...
    it("should cache responses when cache is enabled", async () => {
      ...
      cachedMocks.mockDownloader.download.mockResolvedValue(Buffer.from(JSON.stringify(latestReleaseFixture)));
  ...
  describe("authentication", () => {
    it("should include token in headers when configured", async () => {
      ...
      authedMocks.mockDownloader.download.mockResolvedValue(Buffer.from(JSON.stringify(latestReleaseFixture)));

      await authedMocks.apiClient.getLatestRelease("Codeberg", "pages-server");
      expect(authedMocks.mockDownloader.download).toHaveBeenCalledWith(expect.anything(), expect.anything(), {
        headers: {
          Accept: "application/json",
