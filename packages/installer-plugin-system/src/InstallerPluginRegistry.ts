...
export class InstallerPluginRegistry {
  ...
  async register<T extends InstallerPlugin>(plugin: T): Promise<void> {
    ...
    try {
      ...
      this.logger.info(messages.pluginRegistered(method, plugin.displayName, plugin.version));
    } catch (error) {
  ...
  async install(
    ...
    return await plugin.install(toolName, toolConfig, context, options, logger);
  }
