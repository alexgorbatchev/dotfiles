...
describe('InstallerPluginRegistry - TypeScript Integration', () => {
  ...
  test('ToolConfigBuilder with custom plugin method - TypeScript types work correctly', async () => {
    ...
    const customPlugin: InstallerPlugin<
      'custom-npm',
      CustomNpmParams,
      CustomNpmConfig,
      { npmVersion: string }
    ...
    // With module augmentation, TypeScript now enforces types at compile time!
    // These lines demonstrate compile-time type safety - they cause TypeScript errors:

    // @ts-expect-error - No overload matches this call - foo is not a valid parameter
    builder.bin('test').install('custom-npm', { packageName: '@myorg/tool', foo: 123 });

    // @ts-expect-error - No overload matches this call - packageName is required
    builder.bin('test').install('custom-npm', { registryUrl: 'https://npm.example.com' });
  });
});
