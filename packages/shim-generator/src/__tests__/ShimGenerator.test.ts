...
describe('ShimGenerator', () => {
  ...
  describe('externally managed tools', () => {
    ...
    it('should create symlink instead of bash shim for externally managed tools', async () => {
      ...
      // Clear mocks after construction
      ...
      const result = await brewShimGenerator.generateForTool(toolName, toolConfig);

      expect(result).toEqual([expectedShimPath]);
      // Should create symlink, not write a file
      expect(fsMocks.symlink).toHaveBeenCalledWith(expectedBinaryPath, expectedShimPath);
      expect(fsMocks.writeFile).not.toHaveBeenCalled();
      expect(fsMocks.chmod).not.toHaveBeenCalled();
    });

    it('should create bash shim when tool is not externally managed', async () => {
      const nonBrewConfig: ToolConfig = {
        name: 'github-tool',
        binaries: ['github-tool'],
        version: '1.0.0',
        installationMethod: 'github-release',
        installParams: { repo: 'owner/repo' },
      };
      ...
      const result = await brewShimGenerator.generateForTool('github-tool', nonBrewConfig);

      expect(result).toHaveLength(1);
      // Should write a bash shim, not create a symlink
      expect(fsMocks.writeFile).toHaveBeenCalledTimes(1);
      expect(fsMocks.symlink).not.toHaveBeenCalled();
    });

    it('should overwrite existing bash shim with symlink when tool becomes externally managed', async () => {
      const externallyManagedMethods = new Set(['brew']);
      const brewShimGenerator = new ShimGenerator(logger, fileSystem, mockConfig, systemInfo, externallyManagedMethods);
      const expectedShimPath = path.join(mockConfig.paths.targetDir, toolName);
      const expectedBinaryPath = path.join(mockConfig.paths.binariesDir, toolName, 'current', toolName);

      // Write an existing bash shim first
      await fileSystem.ensureDir(mockConfig.paths.targetDir);
      await fileSystem.writeFile(expectedShimPath, '#!/usr/bin/env bash\n# Generated by Dotfiles Management Tool');
      ...
      const result = await brewShimGenerator.generateForTool(toolName, toolConfig, { overwrite: true });

      expect(result).toEqual([expectedShimPath]);
      expect(fsMocks.symlink).toHaveBeenCalledWith(expectedBinaryPath, expectedShimPath);
    });

    it('should recognize existing symlink pointing to binaries dir as our shim', async () => {
      const externallyManagedMethods = new Set(['brew']);
      const brewShimGenerator = new ShimGenerator(logger, fileSystem, mockConfig, systemInfo, externallyManagedMethods);
      const expectedShimPath = path.join(mockConfig.paths.targetDir, toolName);
      const expectedBinaryPath = path.join(mockConfig.paths.binariesDir, toolName, 'current', toolName);

      // Create an existing symlink pointing to our binaries dir
      // The target must exist for the mem FS to report the symlink as existing
      await fileSystem.ensureDir(path.dirname(expectedBinaryPath));
      await fileSystem.writeFile(expectedBinaryPath, '#!/bin/bash\necho "test"');
      await fileSystem.ensureDir(mockConfig.paths.targetDir);
      ...
      // Without overwrite, should skip since it's our shim
      const result = await brewShimGenerator.generateForTool(toolName, toolConfig, { overwrite: false });

      expect(result).toEqual([]);
      expect(fsMocks.symlink).not.toHaveBeenCalled();
    });

    it('should create symlink for each binary of an externally managed tool', async () => {
      const multiConfig: ToolConfig = {
        name: 'multi-brew',
        binaries: ['bin-a', 'bin-b'],
        version: '1.0.0',
        installationMethod: 'brew',
        installParams: { formula: 'multi-brew' },
      };
      const externallyManagedMethods = new Set(['brew']);
      const brewShimGenerator = new ShimGenerator(logger, fileSystem, mockConfig, systemInfo, externallyManagedMethods);
      ...
      const result = await brewShimGenerator.generateForTool('multi-brew', multiConfig);

      expect(result).toHaveLength(2);
      expect(fsMocks.symlink).toHaveBeenCalledTimes(2);
      expect(fsMocks.symlink).toHaveBeenCalledWith(
        path.join(mockConfig.paths.binariesDir, 'multi-brew', 'current', 'bin-a'),
        path.join(mockConfig.paths.targetDir, 'bin-a'),
      );
      expect(fsMocks.symlink).toHaveBeenCalledWith(
        path.join(mockConfig.paths.binariesDir, 'multi-brew', 'current', 'bin-b'),
        path.join(mockConfig.paths.targetDir, 'bin-b'),
      );
    });
    ...
    it('should default to empty set when no externallyManagedMethods provided', async () => {
      ...
      // Even brew tools should get bash shims when no methods are provided
      const result = await defaultGenerator.generateForTool(toolName, toolConfig);
