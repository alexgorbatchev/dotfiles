...
import type { AptToolConfig } from "../schemas";
...
function createMockContext(toolConfig: AptToolConfig, mockShell: IShell): IInstallContext {
  return {
    projectConfig: {
      paths: {
        binariesDir: "/bin",
...
describe("installFromApt", () => {
  ...
  it("installs latest package without updating package lists by default", async () => {
    const commands: string[] = [];
    const results: MockShellResult[] = [
      createShellResult(""),
      createShellResult("/usr/bin/rg\n"),
      createShellResult("13.0.0-1"),
    ];
    ...
    const result = await installFromApt("ripgrep", toolConfig, context, undefined, logger, shell, shell);
  ...
  it("installs an exact package version and updates package lists when requested", async () => {
    const commands: string[] = [];
    const results: MockShellResult[] = [createShellResult(""), createShellResult(""), createShellResult("")];
    const shell = createMockShell((cmd) => {
      commands.push(cmd);
      const result = results.shift();
      assert(result);
  ...
  it("runs update and install through sudo when configured", async () => {
    ...
    expect(spawnMock.mock.calls).toEqual([
      [
        {
          cmd: ["sudo", "-p", "Please enter your password to continue:", "--", "apt-get", "update"],
          cwd: "/staging/dir",
          env: process.env,
