...
export const extractorLogMessages = {
  shellCommandStarted: (command: string) =>
    createSafeLogMessage(`Executing shell command: ${command}`),
  ...
  extractionRequested: (archivePath: string) =>
    createSafeLogMessage(`Extracting archive ${archivePath}`),
  executableFlagApplied: (filePath: string) =>
    createSafeLogMessage(`Marked file as executable: ${filePath}`),
