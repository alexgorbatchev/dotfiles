...
export class ShimGenerator implements IShimGenerator {
  ...
  async generateForTool(
    ...
    // This ensures it works regardless of where the generator is installed
    const currentModulePath = fileURLToPath(import.meta.url);
    const currentModuleDir = path.dirname(currentModulePath);
    // Navigate from current module directory (generator/src/modules/generator-shim) to scripts directory
    const installToolScriptPath = path.join(
      currentModuleDir, // generator/src/modules/generator-shim
      '..', // generator/src/modules
      '..', // generator/src
      'scripts',
      'install-tool.sh'
    );
    log('generateForTool: installToolScriptPath=%s', installToolScriptPath);
...
INSTALL_TOOL="${installToolScriptPath}"
...
if [ -x "$TOOL_EXECUTABLE" ]; then
  ...
  "$INSTALL_TOOL" "${toolName}" "${cliToolPath}"
