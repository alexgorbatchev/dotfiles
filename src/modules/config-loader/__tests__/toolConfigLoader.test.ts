...
describe('loadToolConfigs', () => {
  let mockFs: IFileSystem & {
    readdir: ReturnType<typeof mock<IFileSystem['readdir']>>;
    // readFile is part of IFileSystem, ensure mock matches a possible signature
    readFile: ReturnType<
      typeof mock<(path: string, encoding?: BufferEncoding) => Promise<string | Buffer>>
    >;
  };
  ...
  beforeEach(() => {
    mockFs = {
      ...createMockFileSystem({ [MOCK_TOOL_CONFIGS_DIR]: null }), // Ensure dir exists
      readdir: mock(async (_path: string) => []),
      // Ensure the mock for readFile is compatible with IFileSystem['readFile']
      readFile: mock(async (_path: string, encoding?: BufferEncoding): Promise<string | Buffer> => {
        // This mock can be more specific if needed for certain tests,
        // but for a generic default, returning an empty string or Buffer is fine.
        if (encoding) return Promise.resolve('');
        return Promise.resolve(Buffer.from(''));
      }) as any, // Cast to any to bypass complex overload type checking for the mock
    };
  ...
  it('should return an empty object if no .tool.ts files are found', async () => {
    mockFs.readdir.mockResolvedValueOnce(['file1.txt', 'another.js']);
  ...
  it('should skip files with invalid ToolConfig structure (failing Zod validation)', async () => {
    ...
    mockFs.readdir.mockResolvedValueOnce(['invalid.tool.ts']);
  ...
  it('should handle dynamic import errors gracefully', async () => {
    mockFs.readdir.mockResolvedValueOnce(['errorimport.tool.ts']);
  ...
  it('should skip tool file if it has no default export', async () => {
    mockFs.readdir.mockResolvedValueOnce(['nodefault.tool.ts']);
