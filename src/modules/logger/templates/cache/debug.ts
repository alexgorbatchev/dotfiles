...
export const cacheDebugTemplates = {
  disabled: (operation: string, key: string) => createSafeLogMessage(`Cache disabled, ${operation} for key: ${key}`),
  notFound: (key: string) => createSafeLogMessage(`No cache entry found for key: ${key}`),
  expired: (key: string) => createSafeLogMessage(`Cache entry expired for key: ${key}`),
  binaryFileMissing: (key: string, path: string) =>
    createSafeLogMessage(`Binary file missing for key: ${key}, path: ${path}`),
  directoryNotExist: () => createSafeLogMessage('Cache directory does not exist, nothing to clear'),
