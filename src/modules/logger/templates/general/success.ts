...
export const generalSuccessTemplates = {
  ...
  operationComplete: (operation: string): SafeLogMessage => createSafeLogMessage(`${operation} completed successfully`),
  cleanup: (resource: string): SafeLogMessage => createSafeLogMessage(`Cleaned up ${resource}`),
  validated: (item: string): SafeLogMessage => createSafeLogMessage(`${item} validated successfully`),
} as const;