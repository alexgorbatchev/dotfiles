...
export const commandDebugTemplates = {
  ...
  githubApiError: (toolName: string) => 
  ...
  installDebug: (operation: string) => 
    createSafeLogMessage(`install: ${operation}`),
  installMethodDebug: (method: string, operation: string) => 
    createSafeLogMessage(`${method}: ${operation}`),
  hookExecution: (hookName: string) => 
