...
export const commandErrorTemplates = {
  ...
  notFound: (command: string) => createSafeLogMessage(`Command not found: ${command}`),
  permissionDenied: (command: string) => createSafeLogMessage(`Permission denied executing: ${command}`),
  ...
  interruptedByUser: (command: string) => createSafeLogMessage(`Command interrupted by user: ${command}`),
