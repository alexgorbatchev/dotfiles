...
describe('Profile Updates E2E Tests', () => {
  ...
  describe('Complete Profile Update Workflow', () => {
    it('should generate shell scripts and update all existing profile files', async () => {
      ...
      expect(bashContent.indexOf('export PS1=')).toBeLessThan(bashContent.indexOf('# Generated via dotfiles generator'));
      expect(psContent.indexOf('$PSDefaultParameterValues')).toBeLessThan(psContent.indexOf('# Generated via dotfiles generator'));
    });
    ...
    it('should generate comprehensive shell script content and update profiles for real-world scenario', async () => {
      ...
      await mockFileSystem.ensureDir(path.dirname(bashProfile));
      await mockFileSystem.writeFile(bashProfile, dedentString(`
        # Bash Configuration
        shopt -s histappend
        HISTSIZE=10000
        HISTFILESIZE=20000
      ...
      expect(zshScriptContent).toContain('# =========================== Shell Completions Setup ===========================');
