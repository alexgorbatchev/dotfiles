...
describe('ShellInitGenerator', () => {
  ...
  it('should include tool-specific initializations from zshInit', async () => {
    ...
    expect(content).toContain('# Tool: toolB');
  ...
  it('should handle mixed configurations for multiple tools', async () => {
    ...
    // Tool Inits - only beta should appear since alpha and gamma are hoisted
    expect(content).toContain('# Tool: beta');
  ...
  it('should correctly order the sections', async () => {
    ...
    const initSectionIndex = content.indexOf(createSectionHeader('zsh', 'Tool-Specific Initializations'));
