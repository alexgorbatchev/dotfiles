...
export const shellInitGeneratorLogMessages = {
  ...
  generate: {
    started: (fileSystemName: string) =>
      createSafeLogMessage(`Starting shell init generation using ${fileSystemName}`),
    ...
    processingTool: (toolName: string) =>
      createSafeLogMessage(`Processing shell init configuration for ${toolName}`),
    skippingTool: (toolName: string) =>
      createSafeLogMessage(`Skipping shell init configuration for ${toolName}`),
  ...
  profiles: {
    starting: (entryCount: number) =>
      createSafeLogMessage(`Updating ${entryCount} shell profile entries`),
  } satisfies SafeLogMessageMap,
  cleanup: {
    onceDirectoryMissing: (directoryPath: string) =>
      createSafeLogMessage(`Shell init once directory missing: ${directoryPath}`),
    onceScriptRemoved: (scriptPath: string) =>
      createSafeLogMessage(`Removed stale once script ${scriptPath}`),
