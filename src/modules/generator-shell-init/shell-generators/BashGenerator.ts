 ...
 */
export class BashGenerator implements IShellGenerator {
  ...
  extractShellContent(_toolName: string, toolConfig: ToolConfig): ShellInitContent {
    ...
    if (toolConfig.bashInit && toolConfig.bashInit.length > 0) {
      toolConfig.bashInit.forEach((line) => {
