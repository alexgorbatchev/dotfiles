...
export function registerCleanupCommand(program: Command, services: CleanupCommandServices) {
  program
    .command('cleanup')
    ...
    .action(async (options: CleanupCommandOptions) => {
      // Commander passes options directly.
      // We need to ensure our logger respects verbose/quiet if not handled by a global setup.
      // For now, assuming ConsolaInstance passed in `services` might be pre-configured or cleanupActionLogic handles it.
      if (options.verbose) {
        services.clientLogger.level = 4; // consola debug level
      }
      if (options.quiet) {
        services.clientLogger.level = 1; // consola warn level (errors still show)
      }
