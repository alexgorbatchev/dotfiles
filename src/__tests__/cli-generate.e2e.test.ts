...
describe('E2E: bun run cli generate', () => {
  describe('generating shims, shell init, and symlinks', () => {
    ...
    beforeAll(async () => {
      ...
      zshInitFilePath = path.join(testDirs.paths.completionsDir, 'init.zsh');
      lazygitSourceConfigPath = path.join(
        testDirs.paths.dotfilesDir,
        '02-configs',
        'lazygit',
        'config.yml'
      );
      ...
      expect(fzfContent).toMatchLooseInlineSnapshot`
        ...
        if [ -x "$TOOL_EXECUTABLE" ]; then
          ...
          if [ -x "$TOOL_EXECUTABLE" ]; then
            ...
            echo "Failed to install 'fzf' via '$GENERATOR_CLI_EXECUTABLE' or it's still not found at '$TOOL_EXECUTABLE'. Please check the installation."
            exit 1
          fi
        fi
      `;
      ...
      expect(lazygitContent).toMatchLooseInlineSnapshot`
        ...
        if [ -x "$TOOL_EXECUTABLE" ]; then
          ...
          if [ -x "$TOOL_EXECUTABLE" ]; then
            ...
            echo "Failed to install 'lazygit' via '$GENERATOR_CLI_EXECUTABLE' or it's still not found at '$TOOL_EXECUTABLE'. Please check the installation."
            exit 1
          fi
        fi
      `;
