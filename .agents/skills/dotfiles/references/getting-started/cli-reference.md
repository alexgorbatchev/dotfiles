...
| ------------------ | ---------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `update-available` | `<tool>: update available (<installed> -> <latest>)`, or `<tool>: available (<latest>)` for a tool with no installed version | A newer release counts as an update.                                                                                                                                                               |
...
In agent mode (`AGENT=1`) each tool is one line, `tool:<tool> status:<status> current:<installed> latest:<latest> cached:<true|false>`, or `tool:<tool> status:unsupported current:<installed> installer:<method>`.
...
- `--json`: Output update status in JSON format. Each entry carries `tool`, `status` (one of the statuses above), `currentVersion`, `latestVersion`, and `cached`. `latestVersion` is absent when nothing upstream was asked.
