...
- `tool update --shim-mode` (a shim's `@update`, which prints nothing of its own) keeps bootstrap, orchestrator and per-tool progress quiet but logs each tool's outcome (pinned, up to date, new version, updated, unsupported check) at the default level through `updateLogs.report`. Route new per-tool outcome messages through `report` and progress through `progress`, never by raising a message's level.
