...
- `tool update` on a tool whose installer returns `installer.ErrUpdateCheckUnsupported`: a named tool (and shim `@update`) warns and reinstalls, as v1 did; updating everything warns and skips it unless `--force` is passed. Such a reinstall pins no version unless the configuration does (`resolveUpdate` answers `""`), so the orchestrator records the detected version or a fresh timestamp, never the previous installation's version.
