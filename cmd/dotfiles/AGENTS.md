...
- `tool update` with no arguments skips silently only a tool with no installation method (a configuration-only tool, as `tool check` skips it) or no installation record; a record it cannot read, a failed update check (with or without `--force`) and a failed reinstall are each logged with the cause and the run moves on to the next tool; a failed installer lookup is an internal error (the load rejects unknown methods) and fails the command, as in `tool check`.
...
- `tool check` reads each tool's installation record and classifies it through `orchestrator.CheckTool`; `ToolUpdateResult.Status` (`--json` `status`, agent-mode `status:`) is that `orchestrator.CheckStatus`. A tool with no record is `not-installed`, never a failed check; a failed query is logged as `Update check failed: <error>` and left out of the results, so the two stay distinguishable for the exit status (#132). `tool update` never installs a release older than the installed version (`ahead-of-latest`): it reports the status, and `--force` reinstalls the installed version.
