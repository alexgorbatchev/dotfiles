...
- `tool check` reads each tool's installation record and classifies it through `orchestrator.CheckTool`; `ToolUpdateResult.Status` (`--json` `status`, agent-mode `status:`) is that `orchestrator.CheckStatus`. A tool with no record is `not-installed`, never a failed check; a failed query is logged as `Update check failed` and left out of the results, so the two stay distinguishable for the exit status (#132). `tool update` never installs a release older than the installed version (`ahead-of-latest`): it reports the status, and `--force` reinstalls the installed version.
