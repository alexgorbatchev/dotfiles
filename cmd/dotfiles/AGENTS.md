...
- `tool update` refuses a tool whose `.version()` is anything other than `"latest"` (`config.ToolConfig.UpdateRefusal`, shared with the dashboard's update route) before its update check, named or batch, with or without `--force`, as v1 did; `tool check` still reports newer releases for it.
