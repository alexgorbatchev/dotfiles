...
- `tool update` on a tool whose installer returns `installer.ErrUpdateCheckUnsupported`: a named tool (and shim `@update`) warns and reinstalls, as v1 did; updating everything warns and skips it unless `--force` is passed.
