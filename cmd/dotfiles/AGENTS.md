...
- `tool update` with no arguments skips silently only a disabled tool (as install and `tool check` skip it), a tool with no installation method (a configuration-only tool, as `tool check` skips it) or no installation record; a record it cannot read, a failed update check (with or without `--force`) and a failed reinstall are each logged with the cause and the run moves on to the next tool, then returns `ErrSilent` so the command exits 1 (#132); a failed installer lookup is an internal error (the load rejects unknown methods) and fails the command, as in `tool check`.
- `tool update` on a tool whose installer returns `installer.ErrUpdateCheckUnsupported`: a named tool (and shim `@update`) warns and reinstalls, as v1 did; updating everything warns and skips it unless `--force` is passed. Such a reinstall pins no version (`orchestrator.PlanUpdate` answers an empty `TargetVersion`), so the orchestrator records the detected version or a fresh timestamp, never the previous installation's version.
- `tool update` refuses a tool whose requested version (`config.ToolConfig.RequestedVersion`: a `version` install parameter the method reads, else `.version()`) is anything other than `"latest"` (`config.ToolConfig.UpdateRefusal`, shared with the dashboard's update route) before its update check, named or batch, with or without `--force`, as v1 did, and exits 0; `tool check` still reports newer releases for it.
- `tool check` reads each tool's installation record and classifies it through `orchestrator.CheckTool`; `ToolUpdateResult.Status` (`--json` `status`, agent-mode `status:`) is that `orchestrator.CheckStatus`. A tool with no record is `not-installed`, never a failed check; a failed query is logged as `Update check failed: <error>` (an unreadable record as `Reading the installation record failed: <error>`), left out of the plain and agent-mode lines, listed in `--json` as `checkStatusFailed` (`status: "failed"`, a CLI-only status: `orchestrator.CheckTool` returns the error instead) with `error`, and after every other tool is checked (and the JSON printed) the command returns `ErrSilent` and exits 1 (#132). `not-installed`, `unsupported`, `ahead-of-latest` and pinned refusals are never failures. `tool update` never installs a release older than the installed version (`ahead-of-latest`): it reports the status, and `--force` reinstalls the installed version.
- `tool update --shim-mode` (a shim's `@update`, which prints nothing of its own) keeps bootstrap, orchestrator and per-tool progress quiet but logs each tool's outcome (pinned, up to date, ahead of the latest release, new version, updated, unsupported check) at the default level through `updateLogs.report`. Route new per-tool outcome messages through `report` and progress through `progress`, never by raising a message's level.
