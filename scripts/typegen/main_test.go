...
func TestMainFunc(t *testing.T) {
	...
	// runMain with no arguments writes to a path relative to the working directory,
	// so it has to be run from a temporary one. Left alone it creates
	// packages/dashboard/src/shared/types.gen.ts inside this package's own directory,
	// which both litters the tree and, being newer than the test binary, stops Go
	// from ever caching this package's result.
	originalWd, err := os.Getwd()
	if err != nil {
		t.Fatalf("failed to get current working dir: %v", err)
	}
	...
	defaultPath := filepath.Join(tmpDir, "packages", "dashboard", "src", "shared", "types.gen.ts")
	if _, err := os.Stat(defaultPath); err != nil {
		t.Fatalf("expected runMain() default to write %s: %v", defaultPath, err)
	}
}
