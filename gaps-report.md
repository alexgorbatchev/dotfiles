...
- Core execution engine, CLI subcommands, all 15 installer plugins, registry database, filesystem abstractions, Goja VM loader, dashboard server, and build pipeline reach 96% functional and output parity.
- The remaining 0.4 score gap represents:
  - Permission representation string conversions (`OctalToDecimalPerm` / `DecimalToOctalPerm`) in `pkg/registry/registry.go`.
  - Chronological file state attribute accumulation across `chmod` operations in `GetFileState`.
  - `MemFS.CopyFile` symlink dereferencing and `MemFS.ReadDir` error return parity in `pkg/fs/mem_fs.go`.
  - HTTPS tunneling (`CONNECT` method TCP hijacking) in local caching proxy `pkg/proxy/proxy.go`.
...
| :--------- | :------------------ | :---------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--------- | :-------------------------------------------------------------------------------------------------------------------------- |
| **DUE-1**  | **Blocker**         | `pkg/registry/registry.go:587`                                    | `OctalToDecimalPerm("420")` treats decimal `"420"` (which is `0644` in octal) as octal `0420` and converts to `"272"`. `DecimalToOctalPerm("0755")` parses `"0755"` as base-10 `755` and formats to `"01363"`. | **OPEN**   | Check string prefixes (`0o`, `0`) and handle base-10 vs octal representation correctly.                                     |
| **DUE-2**  | **Blocker**         | `pkg/registry/registry.go:277`                                    | `GetFileState` inspects only `ops[0]` (newest operation). If latest operation was `chmod`, attributes like `targetPath` or `sizeBytes` recorded during prior `writeFile` or `symlink` are lost (`nil`).        | **OPEN**   | Iterate chronologically from oldest to newest operation to accumulate active file state.                                    |
...
| **DUE-4**  | **Medium**          | `pkg/fs/mem_fs.go:420`                                            | When `src` is a symlink, `MemFS.CopyFile` creates `dest` as a symlink node pointing to `linkTarget` rather than copying target file contents.                                                                  | **OPEN**   | Dereference symlinks in `MemFS.CopyFile` to copy target file bytes when `src` is a symlink.                                 |
...
| **DUE-26** | **Medium**          | `pkg/proxy/proxy.go:412`                                          | Local caching proxy HTTPS `CONNECT` method handling.                                                                                                                                                           | **OPEN**   | Implement TCP connection hijacking for `CONNECT` method in `proxy.go`.                                                      |
...
| **DUE-30** | **Medium**          | `pkg/fs/mem_fs.go:260`                                            | `MemFS.ReadDir` error return for missing paths.                                                                                                                                                                | **OPEN**   | Return `os.ErrNotExist` when target directory path is missing in `m.files`.                                                 |
