...
- Resolve a `binaryPath` install parameter only through `resolveBinaryPath` (`binary_path.go`), which `manual` and `curl-script` share so the same value cannot mean different paths per method.
