...
func TestHeaderFormatting(t *testing.T) {
	t.Run("HeaderLine centered title", func(t *testing.T) {
		got := HeaderLine("=", "Dotfiles CLI")
		expected := "# ================================ Dotfiles CLI ================================="
		if got != expected {
			t.Errorf("HeaderLine() = %q, want %q", got, expected)
		}
	})

	t.Run("GenerateFileHeader", func(t *testing.T) {
		got := GenerateFileHeader("/home/user/.dotfiles")
		if !strings.Contains(got, "THIS FILE IS AUTOMATICALLY GENERATED BY THE DOTFILES MANAGEMENT TOOL") {
			t.Errorf("expected header to contain warning text, got:\n%s", got)
		}
		if !strings.Contains(got, "# Dotfiles directory: /home/user/.dotfiles") {
			t.Errorf("expected header to contain dotfiles directory, got:\n%s", got)
		}
	})

	t.Run("GenerateEndOfFile", func(t *testing.T) {
		got := GenerateEndOfFile()
		expected := "# ============================ End of Generated File ============================"
		if got != expected {
