...
- Ensure async tool callbacks return the underlying `builder` rather than a raw JS `Promise` object. Because the promise is not what `defineTool` returns, nothing else observes it: `recordToolFactory` keeps it in `__toolFactories` and `settleToolFactories` settles it in Go once the bundle has run, so a factory that rejects fails the load naming its file instead of dropping the tool in silence.
