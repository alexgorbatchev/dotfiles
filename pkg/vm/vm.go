...
func stripImports(scriptContent string) string {
	...
	transpiled, err := transpileTS(cleaned)
	if err == nil && transpiled != "" {
		return transpiled
	}
	return cleaned
}
...
func EvaluateToolDefinition(scriptContent string, out any) error {
	return EvaluateToolDefinitionWithContext(scriptContent, "", nil, out)
}
...
func EvaluateToolDefinitionWithContext(scriptContent string, configDir string, sysCtx *SystemContext, out any) error {
	...
	// Run bootstrap JS
	if _, err := vm.RunString(loaderApiContents); err != nil {
