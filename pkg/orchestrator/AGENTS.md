...
- Detect cross-tool conflicts (aliases shadowing binaries, functions shadowing binaries, alias/function/binary collisions) and warn during generation (`WarnConflicts`) and validation (`DetectConflicts`). Two declarations writing one file are not a warning: `config.ValidateToolConfigs` rejects them at load, so the pipelines never see them.
