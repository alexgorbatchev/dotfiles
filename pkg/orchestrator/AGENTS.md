...
- `update.go` owns the update decision shared by the CLI's `tool update` and the dashboard's update route: `PlanUpdate` turns an installer's `CheckUpdate` answer into an `UpdatePlan` (a failed check is an error, `installer.ErrUpdateCheckUnsupported` a reinstall without a target) and `ApplyUpdate` reinstalls with force from a copy of the tool configuration. Callers must not reimplement either.
