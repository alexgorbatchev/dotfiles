...
func (sc *ShadowChecker) resolveBinaryTarget(ctx context.Context, tool *config.ToolConfig, binName string, projCfg *config.ProjectConfig) string {
...
func (sc *ShadowChecker) CheckTool(ctx context.Context, tool *config.ToolConfig, projCfg *config.ProjectConfig) []ShadowWarning {
	...
	for _, binName := range binNames {
		if extPath, found := sc.findExternalCommand(binName, projCfg); found {
			...
			resolvedTarget := sc.resolveBinaryTarget(ctx, tool, binName, projCfg)
			if resolvedTarget != "" && resolvedTarget == extPath {
				continue
			}
			if tool.InstallationMethod == "manual" {
				manualPath := getStringParam(tool.InstallParams, "binaryPath", "")
				if manualPath != "" && (manualPath == extPath || manualPath == binName) {
					continue
				}
			}
