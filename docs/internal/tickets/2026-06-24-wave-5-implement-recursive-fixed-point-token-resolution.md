...
- [ ] Implement a recursive fixed-point token resolver in Go (e.g. inside `pkg/config` or `pkg/orchestrator`) that replaces placeholders with their corresponding paths from `ProjectConfig`.
- [ ] Set a safe maximum iteration limit (e.g. 20 iterations) and throw a descriptive error if a cyclic token reference is detected.
- [ ] Refactor `resolvePlaceholder` in `pkg/orchestrator/orchestrator.go` to leverage this new recursive token resolver.
- [ ] Write unit tests verifying that nested path variables (such as `{paths.generatedDir}/user-home` referencing `{paths.homeDir}`) are fully resolved and that cyclic references fail fast with the expected error.
