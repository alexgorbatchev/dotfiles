...
- [x] Refactor `pkg/installer/dmg.go` and `pkg/installer/pkg.go` to support a polymorphic `source` parameter matching TS schemas.
- [x] Integrate GitHub Release resolving logic inside Go's `dmg` and `pkg` installers to fetch download assets dynamically from GitHub releases on demand.
- [x] Implement compressed archive extraction (e.g. `.zip`, `.tar.gz`) support for compressed PKG/DMG files.
- [x] Write unit tests inside `pkg/installer/dmg_test.go` and `pkg/installer/pkg_test.go` verifying the correct resolution, extraction, and installation of complex DMG/PKG configurations.
