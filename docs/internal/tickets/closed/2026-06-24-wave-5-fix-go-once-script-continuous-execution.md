...
- [x] Refactor once-script generation inside `pkg/orchestrator/orchestrator.go` (line 586-611) to append self-deletion commands at the end of written once-scripts:
  - POSIX shells (zsh, bash): Append `\nrm -f "$0"\n` or similar self-cleanup hook.
  - PowerShell: Append `\nRemove-Item $MyInvocation.MyCommand.Path -ErrorAction SilentlyContinue\n` or similar.
- [x] Ensure that existing files in `.once/` are safely pruned/managed during consecutive generate commands.
- [x] Write unit tests inside `pkg/orchestrator/orchestrator_test.go` verifying that written once-scripts contain the appropriate self-deletion statements.
- [x] Run `bun check` and `bun check:ci` to verify that Go shell init tests pass cleanly.
