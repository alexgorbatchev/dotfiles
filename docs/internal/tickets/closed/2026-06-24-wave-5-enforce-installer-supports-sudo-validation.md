...
- [x] Edit `pkg/orchestrator/orchestrator.go`'s `InstallTool` to assert that if `tool.Sudo` is `true`, `inst.SupportsSudo()` must also be `true`.
- [x] If `tool.Sudo` is `true` but `inst.SupportsSudo()` is `false`, immediately return a descriptive error (e.g., `installer "npm" does not support sudo installations`).
- [x] Implement a helper in Go's command execution wrapper to validate TTY status or log warnings before executing commands with elevated root privileges.
- [x] Write unit tests inside `pkg/orchestrator/orchestrator_test.go` asserting that trying to install a tool with `sudo: true` on an installer that returns `SupportsSudo() == false` fails immediately with the expected validation error.
